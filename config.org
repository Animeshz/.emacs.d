...
|            | C-x [2 / 3 / 0 / 1]    | Window Split [Down / Right / Switch / Close / Close All Except This] |
...
(the buffer) has a major mode (e.g. rust-mode) and multiple minor modes (e.g. winner-mode) enabled through file
extension or hooks, each of these modes have a mode-map associated in order to set keybinds in buffers with those modes
enabled. Read more about them [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Modes.html][here]].
